...
public class ExcHeal implements Executor{
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(!sender.hasPermission("yamastools.cmd.heal")){
			Util.sendMessage(sender, "&cNie masz uprawnien do uzycia tej komendy &8(&7yamastools.cmd.heal&8)");
			return;
		}
		...
		if(args.length == 0){
			...
			if(args.length >= 1){
				...
				if(!p.hasPermission("yamastools.cmd.heal.other")){
					Util.sendMessage(p, "&cNie masz uprawnien do uzycia tej komendy &8(&7yamastools.cmd.heal.other&8)");
					return;
				}
