...
public class ExcClearInventory implements Executor{
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(!sender.hasPermission("yamastools.cmd.clearinv")){
			Util.sendMessage(sender, "&cNie masz uprawnien do uzycia tej komendy &8(&7yamastools.cmd.clearinv&8)");
			return;
		}
		...
		if(args.length == 0){
			...
			if(args.length >= 1){
				...
				if(!p.hasPermission("yamastools.cmd.clearinv.other")){
					Util.sendMessage(p, "&cNie masz uprawnien do uzycia tej komendy &8(&7yamastools.cmd.clearinv.other&8)");
					return;
				}
