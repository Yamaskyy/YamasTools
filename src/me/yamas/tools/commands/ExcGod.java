...
public class ExcGod implements Executor{
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(!sender.hasPermission("yamastools.cmd.god")){
			Util.sendMessage(sender, "&cNie masz uprawnien do uzycia tej komendy &8(&7yamastools.cmd.god&8)");
			return;
		}
		...
		if(args.length == 0){
			...
			if(args.length >= 1){
				...
				if(!p.hasPermission("yamastools.cmd.god.other")){
					Util.sendMessage(p, "&cNie masz uprawnien do uzycia tej komendy &8(&7yamastools.cmd.god.other&8)");
					return;
				}
