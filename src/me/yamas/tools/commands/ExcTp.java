...
public class ExcTp implements Executor{
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(!sender.hasPermission("yamastools.cmd.tp")){
			Util.sendMessage(sender, "&cNie masz uprawnien do uzycia tej komendy &8(&7yamastools.cmd.tp&8)");
			return;
		}
		...
		if(args.length == 0){
			...
			if(args.length == 1){
				...
				if(args.length >= 2){
					...
					if(!p.hasPermission("yamastools.cmd.tp.other")){
						Util.sendMessage(p, "&cNie masz uprawnien do uzycia tej komendy &8(&7yamastools.cmd.tp.other&8)");
						return;
					}
					if(from.equals(to)){
