...
public class ExcChat implements Executor{
	...
	public void execute(CommandSender sender, String[] args) {
		if(!sender.hasPermission("yamastools.cmd.chat")){
			Util.sendMessage(sender, "&cNie masz uprawnien do uzycia tej komendy &8(&7yamastools.cmd.chat&8)");
			return;
		}
		if(args.length == 0){
			...
			if(args.length >= 1){
				if(args[0].equalsIgnoreCase("on")){
					...
					Chat.setChat(true);
					Util.broadcast("&8[&2RC&fDROP&8] &7Chat zostal wlaczony przez: &4" + sender.getName(), true);
					...
					Chat.setChat(false);
					Util.broadcast("&8[&2RC&fDROP&8] &7Chat zostal wylaczony przez: &4" + sender.getName(), true);
					...
					Util.broadcast("&8[&2RC&fDROP&8] &7Chat zostal wyczyszczony przez: &4" + sender.getName(), true);
