...
public class ExcFeed implements Executor{
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(!sender.hasPermission("yamastools.cmd.feed")){
			Util.sendMessage(sender, "&cNie masz uprawnien do uzycia tej komendy &8(&7yamastools.cmd.feed&8)");
			return;
		}
		...
		if(args.length == 0){
			...
			if(args.length >= 1){
				...
				if(!p.hasPermission("yamastools.cmd.feed.other")){
					Util.sendMessage(p, "&cNie masz uprawnien do uzycia tej komendy &8(&7yamastools.cmd.feed.other&8)");
					return;
				}
