...
import me.yamas.tools.utils.StringUtil;
...
public class ExcGameMode implements Executor{
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(!sender.hasPermission("yamastools.cmd.gamemode")){
			Util.sendMessage(sender, "&cNie masz uprawnien do uzycia tej komendy &8(&7yamastools.cmd.gamemode&8)");
			return;
		}
		...
		if(args.length == 0){
			...
			if(args.length == 1){
				...
				if(args.length == 2){
					...
					if(!p.hasPermission("yamastools.cmd.gamemode.other")){
						Util.sendMessage(p, "&cNie masz uprawnien do uzycia tej komendy &8(&7yamastools.cmd.gamemode.other&8)");
						return;
					}
