...
public class ExcFly implements Executor{
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(!sender.hasPermission("yamastools.cmd.fly")){
			Util.sendMessage(sender, "&cNie masz uprawnien do uzycia tej komendy &8(&7yamastools.cmd.fly&8)");
			return;
		}
		...
		if(args.length == 0){
			...
			if(args.length == 1){
				...
				if(!p.hasPermission("yamastools.cmd.fly.other")){
					Util.sendMessage(p, "&cNie masz uprawnien do uzycia tej komendy &8(&7yamastools.cmd.fly.other&8)");
					return;
				}
