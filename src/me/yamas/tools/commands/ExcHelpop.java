...
public class ExcHelpop implements Executor{
	...
	public void execute(CommandSender sender, String[] args) {
		if(!sender.hasPermission("yamastools.cmd.helpop")){
			Util.sendMessage(sender, "&cNie masz uprawnien do uzycia tej komendy &8(&7yamastools.cmd.helpop&8)");
			return;
		}
		if(args.length == 0){
			...
			return;
			 ...
			 if(sender instanceof Player){
				 ...
				 IChatBaseComponent title = ChatSerializer.a("{\"text\":\"" + Util.colored("&2RC&fDROP") + "\"}");
			 ...
			 for(Player admins : Bukkit.getOnlinePlayers()){
				 if(admins.hasPermission("yamastools.cmd.helpop.read")){
					 Util.sendMessage(admins, "&b[HelpOp] &7" + sender.getName() + ": &f" + sb.toString());
				 }
			 }
			 return;
		}
