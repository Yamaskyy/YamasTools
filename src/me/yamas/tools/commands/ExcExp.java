...
public class ExcExp implements Executor{
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(!sender.hasPermission("yamastools.cmd.exp")){
			Util.sendMessage(sender, "&cNie masz uprawnien do uzycia tej komendy &8(&7yamastools.cmd.exp&8)");
			return;
		}
			...
			if(args.length == 2){
				if(args[0].equalsIgnoreCase("giveall")){
					...
					for(Player targets : Bukkit.getOnlinePlayers()){
						...
						Util.broadcast("&8[&2RC&fDROP&8] &7Administrator: &4" + p.getName() + "&7 rozdal calemu serwerowi: &4EXP +" + exp, true);
					}
				}
			}
			if(args.length >= 3){
